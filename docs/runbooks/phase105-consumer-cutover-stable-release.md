...
`qdl-v1-fallback:v1.2.4-2b0dcf7` is only the declared V1 image reference. Before a runtime
handoff, the 10.5-B/C packet must verify the running V1 image/source label or
immutable digest against the frozen `v1.2.4` commit; a mutable tag alone is not
release evidence.
...
6. A V1 digest-to-commit attestation for the frozen `v1.2.4` fallback before
...
`data_layer_service` from the frozen `v1.2.4` source. Preserve the original
