...
**R1 bounded-memory retry generation - 2026-08-24 (approved, in progress).**

- The prior real R1 canary is terminalized/fenced before this retry; no failed candidate, cursor, group, authority row or image is reused. V1 remains the hot rollback route, generic scope-fenced Rust cores remain limited to the four non-promoted DNSE bindings, and production_core_1..3 remain stopped until the new candidate is admitted.
- In-scope source repair is commit 2d758ae96fbe77971dc5b5905e376c225e036c00 (fix(kafka): bound client buffers for core recovery). The provider-neutral shared Rust Kafka transport now explicitly bounds client queues/fetches: queued.max.messages.kbytes=8192, fetch.max.bytes=4194304, max.partition.fetch.bytes=1048576, queue.buffering.max.kbytes=8192 and queue.buffering.max.messages=4096. This applies uniformly to every Rust transport client (consumer, transactional producer, authority watcher and compacted recovery reader); it does not change public schemas, V1 or a venue contract.
- Source acceptance passed before runtime mutation: Rust cargo fmt --check, Clippy with warnings denied, qdl-kafka 19 tests (including the explicit bounded-memory policy assertion), production-core 3 tests, realtime-core 2 tests and native-ingestor 6 tests; the pinned Python R0/R1/R2 matrix passed 80/80 with one documented isolated-Redis skip; C3 authority-DB smoke and R1 rollover smoke passed with production_mutations=0.
- Built immutable candidate sha256:9b8daa5e45fc1cbd3c183af93e502c26b4cbcdcff1bbb0ea71e9e4ddb476df2b (qdl-v2-rust:2.0.0-2d758ae96fbe, OCI revision 2d758ae96fbe77971dc5b5905e376c225e036c00, UID/GID 10001:10001). Retained Rust rollback image is sha256:51a64da92c884b5d8031e5e1593e635c760a9c14f837467d077459b41248c23d; V1 remains a separate hot fallback.
- Created a new sealed generation-bound bundle r1-9b8daa5e45fc-membounds-20260824T070039Z without overwriting prior evidence. Read-only preflight verified root 0700, stable.env 0600, public production runtime manifest 0644, and private release evidence remains non-world-readable. The new group/key generation is distinct from all earlier attempts; no Kafka offset was reset, sought, committed, deleted or reused.
- Remaining approved R1/R2 sequence: verify exact non-root runtime inputs; collect a fresh authentic 12-slice reference report and candidate-bound admission; prepare/dry-run/apply one append-only BLOCKED rollover and C3 VALIDATING then RUST_CANARY packets; issue a new signed tail cursor without offset mutation; start each production core one at a time only after the preceding core has structured recovery, no OOM/authority/transport error and bounded RSS/lag; collect 5-10 minutes of real provider parity/handoff evidence; only then prepare and apply the exact R2 RUST_CANARY -> RUST_PRIMARY CAS packet. Any failure stops production_core_1..3, fences authority with C3 and retains every audit record; it never restarts V1, resets Kafka, flushes Redis or modifies Trading System/DNSE/Spot routes.

**R1 control-plane stable-env alias repair - 2026-08-24 (in progress).**

- Fresh real 12-slice reference evidence passed before any authority mutation: 96 authentic samples (at least 8 per slice), zero semantic mismatch, invalid provenance, gap, duplicate or consumer error, and production_mutations=0. Candidate-bound pre-canary admission passed for sha256:9b8daa5e45fc1cbd3c183af93e502c26b4cbcdcff1bbb0ea71e9e4ddb476df2b against distinct reference image sha256:db240925dff30d4b9deb338dbd8e6e3506cbc8501ee71cff09ff58a247b7bae6; the immutable C40 bootstrap packet passed review with twelve slices and no mutation.
- The first rollover prepare stopped fail-closed because sealed stable bundles expose QDL_STABLE_CONTROL_ADMIN_DSN, while R1/R2 operator scripts only accepted the unprefixed runtime alias QDL_CONTROL_ADMIN_DSN. This is an operator-contract/configuration defect, not a database/Kafka failure; no packet was applied and no authority state changed.
- Repair scope: introduce one shared control-plane admin-DSN resolver used by C3 bootstrap/cutover, C40 handoff, R1 rollover/transition and R2 primary preparation. It accepts either declared alias, rejects a conflicting dual declaration without echoing values, and preserves the unprefixed runtime alias for external operators. Focused unit/CLI regression and the fresh packet flow must pass before retrying rollover.

**R1 control-plane stable-env alias repair - 2026-08-24 (complete source slice).**

- Added qdl.control.operator_env.require_control_admin_dsn and applied it uniformly to R1 candidate rollover/transition, R2 primary preparation, C3 transition, C40 bootstrap and C40 terminal-handoff apply paths. Runtime alias QDL_CONTROL_ADMIN_DSN remains supported; sealed-bundle QDL_STABLE_CONTROL_ADMIN_DSN is now a first-class equivalent. A dual declaration with unequal values stops before a connection and errors without echoing either secret.
- Verification passed in the immutable network-isolated Python image: 40 focused R1/R2/C3/C40/cursor tests passed in 5.449 seconds; seven changed entrypoints passed non-writing syntax compilation. The prior direct py_compile attempt was intentionally not counted because the source bind mount is read-only and Python attempted a cache write. Git whitespace check passed. The empty shell artifact from the earlier malformed journal command was removed.
- Runtime impact remains zero for this source slice: no C3/CAS/rollover apply, Kafka offset reset/seek, Redis/SQLite mutation, V1/Trading System restart or production-core start occurred. Existing fresh reference/admission/bootstrap evidence remains isolated and will be revalidated by packet expiry/CAS checks before apply.

**R1 prerequisite TTL nesting defect - 2026-08-24 (in progress).**

- Read-only inspection of the fresh bootstrap, current BLOCKED rows and rollover parser identified a real packet-construction defect. Bootstrap and rollover were both requested with TTL 1800 seconds, while CandidateRolloverPacket requires the child expiry to be no later than its prerequisite-bundle expiry. Any nonzero delay between preparing parent and child therefore makes the child impossible to parse.
- This is not an approval relaxation. The repair will derive the child expiry as the earlier of requested child TTL and immutable parent bundle expiry, and fail before packet output when less than 60 seconds of parent approval remains. Thus a rollover can never outlive the admission/bootstrap that authorizes it.
- Add boundary tests for clamped child expiry and nearly-expired parent failure. Rebuild fresh admission/bootstrap/rollover evidence after the source test; no authority, Kafka offset, Redis, V1 or production core mutation is permitted until then.

**R1 prerequisite TTL nesting repair - 2026-08-24 (complete source slice).**
